...
while True:
    ...
    title = doc.xpath("/html/body/div[1]/div[1]")[0].text_content()
    # title = remove_space_line(doc.xpath("/html/body/div[1]/div[1]")[0].text_content())
    # content = remove_space_line(doc.xpath("//div[@id='nr']//div")[0].text_content())
    # content = remove_space_line(doc.xpath("//*[@id='pt-pop']")[0].text_content())
    content = doc.xpath("(//p[@class='content_detail'])")
    page_content = ''
    for line_content in content:
        page_content += line_content.text_content()+'\n'
    # print(page_content)
    # content = doc.xpath("(//p[@class='content_detail'])")[0].text_content()
    # content = doc.xpath("/html/body/div[1]/div[3]/div/*")[0].text_content()
    ...
    if base_url.find('htm') == -1:
        ...
        next_click_text = remove_space_line(doc.xpath("/html/body/div[1]/div[5]/div/div[3]/a")[0].text_content())
        ...
        if '/' in doc.xpath("/html/body/div[1]/div[5]/div/div[3]/a")[0].get('href'):
            ...
            base_url = url_temp + doc.xpath("(//a[contains(.,'下一页')])[2]")[0].get('href')
            # print(base_url)
