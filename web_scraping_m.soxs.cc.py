...
base_url = 'https://m.soxs.cc/JianTuZhiLu/3604561.html'
x = 0
...
while True:
    ...
    # # result = EC.alert_is_present()(driver)k
    next_page_xpath = "(//a[contains(.,'下一章')])[2]"
    # confirm_next_page_xpath = next_page_xpath+"[contains(text(),'下一')]"
    # print(confirm_next_page_xpath)
    # print(driver.current_url, driver.title)

    try:
        driver.get(base_url)
        # next_page = driver.find_element_by_xpath(confirm_next_page_xpath).text
        # print('next_page.text: ',next_page)
    ...
    #         break
        ...
        # print(html_str)
        html_str = html_str.replace('喜欢剑徒之路请大家收藏：(m.soxs.cc)剑徒之路搜小说更新速度最快。','')
        ...
        #     text_byte = bytes(html_str,'utf-8')
        #     savetofile.write(text_byte)
        np_url = driver.find_element_by_xpath(next_page_xpath).get_attribute('href')
        # print(np_url)
        # base_url = np_url
        if '.htm' not in np_url:
            print('No more chapters, closing file... done!')
            savetofile.close()
            driver.close()
            driver.quit()
            ...
            # np_button = driver.find_element_by_xpath("(//a[contains(.,'下一章')])[2]")
            # np_button = driver.find_element_by_link_text("下一章")
            np_button = driver.find_element_by_xpath(next_page_xpath)
            np_button.click()
            driver.
            print ("continue...")
        # print('np_url.get_attribute(href): ',np_url)
        # np_url_text = driver.find_element_by_xpath(next_page_xpath).text
        # print('np_url_text: ', np_url_text)
        # break
        # driver.get(np_url)
        # nextpage_button = driver.find_element_by_link_text('下一章')
